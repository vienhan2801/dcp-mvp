...
function getIcon(type: string) {
  ...
  switch (type) {
    ...
    case "order_partially_confirmed": return <AlertTriangle className={cls} />;
...
function getColor(type: string): string {
  switch (type) {
    ...
    case "order_partially_confirmed": return "bg-amber-100 text-amber-700";
          ...
          <div key={log.id} className="flex gap-4 relative">
            ...
            <div className="flex-1 bg-white rounded-xl border border-[#E4EAE7] p-4 shadow-sm">
              <div className="flex items-start justify-between gap-2 flex-wrap">
                ...
                <span className="text-xs text-[#6B7A73] whitespace-nowrap">{fmtDateTime(log.createdAt)}</span>
              </div>
